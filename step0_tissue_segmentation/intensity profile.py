...
def tissue_seg(input_dir, output_dir):
	...
	images_file_names = [file_name for file_name in os.listdir(input_dir) if 'DAPI' in file_name]
	...
	for file_name in images_file_names:
		...
		# binary
		tissue = (im_gray <= intensity_threshold) * 1
		
		tissue = dilation(tissue, selem=disk(5))
		tissue = dilation(tissue, selem=disk(5))
		...
		if len(np.unique(labeled_im)) > 1:
			...
			tissue = np.isin(labeled_im, bcg_label)
			# tissue = dilation(tissue, selem=disk(2))
			# tissue = erosion(tissue, selem=disk(2))
